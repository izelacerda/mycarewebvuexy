...
// import "../../../../assets/scss/especificos/dx.material.mycare-theme.css"
...
export default function FinancialAccountList(props) {
  ...
  useEffect(() =>
  {
      ...
      if (auth.login !== undefined) {
        if (auth.login.licence_id !== undefined) {
          let body = {
            ...
            table_subnumber: config.table_subnumber,
            un_id: 0,
            userlog_id: auth.login.values.loggedInUser.id
          };
    ...
    }, [loadedGeral, config, loadedDados]) // eslint-disable-line
...
//  }
 function onRowPrepared(e) {
  if (e.rowType === 'data' && e.data.is_group) {
    ...
    if(e.data.level === 0 ) {
      e.rowElement.className = e.rowElement.className + " rowColor_00";
    }
    else  if(e.data.level === 1 ) {
      e.rowElement.className = e.rowElement.className + " rowColor_01";
    }
    else  if(e.data.level === 2 ) {
      e.rowElement.className = e.rowElement.className + " rowColor_02";
    }
    else  if(e.data.level === 3 ) {
      e.rowElement.className = e.rowElement.className + " rowColor_03";
    }
    else  if(e.data.level === 4 ) {
      e.rowElement.className = e.rowElement.className + " rowColor_04";
    }
    else  {
      e.rowElement.className = e.rowElement.className + " rowColor_04";
    }
