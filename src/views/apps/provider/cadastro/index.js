    ...
    const toolBarList = [
      {
        ...
        action: () => {  history.push(`/app/provider/cadastro/0`); window.location.reload() }
      },
      ...
      },
      {
        ...
        action: () => history.push(`/app/provider/list`)
      },
      {
        id: 'toolbar5',
        color: 'primary',
    ...
    try {
      ...
      if (!edicao) {
        try {
          ...
          history.push(`/app/provider/cadastro/${id}`)
          toast.success("Prestador incluído com sucesso!", { transition: Flip });
        } catch (error) {
          if (typeof error.response !== 'undefined')
          {
            if(typeof error.response.status !== 'undefined' && (error.response.status === 401 || error.response.status === 400 || error.response.status === 403  ))
            {
              if(error.response.data.message !== undefined) {
                ...
                toast.error(`Erro ao Incluir o prestador! ${error.message}`, { transition: Flip });
              }
            }
          }
          else {
        ...
        try {
          ...
          toast.success("Prestador atualizado com sucesso!", { transition: Flip });
        } catch (error) {
          if (typeof error.response !== 'undefined')
          {
            ...
            toast.error(`Erro ao atualizar o prestador! ${error.message}`, { transition: Flip });
          }
        }
      }
    } catch (error) {
      ...
      if (error instanceof Yup.ValidationError) {
        ...
        toast.error(
          `Não foi possível ${id === "0" ? "incluir" : "alterar"} o prestador. ${error.message}`
        , { transition: Flip });
      }
    }
  }
  ...
  async function handleDelete() {
    try {
      if(id){
        ...
        history.push(`/app/provider/list`)
