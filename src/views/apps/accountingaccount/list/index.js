...
export default function AccountingAccountList(props) {
  const auth = useSelector(state => state.auth);
  let insertPermission = props.userPermission.includes(82+1)
  let deletePermission = props.userPermission.includes(82+3)
  let reportPermission = props.userPermission.includes(82+4)
     ...
     async function loadDados() {
      if (auth.login !== undefined) {
        if (auth.login.licence_id !== undefined) {
          ...
          let response = await api.post("/accountingaccounts.list", {
            ...body
          });
  ...
  async function handleDelete() {
    try {
      if(itemDelete){
        ...
        await api.delete("/accountingaccounts",
          { data }
        );
        setItemDelete(null)
